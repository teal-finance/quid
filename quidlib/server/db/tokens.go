...
func GenNsAdminTokenForUser(userName, nsName string) (string, error) {
	logg.Info("Generating ns admin token for", userName, nsName)
	// get the namespace
	...
	logg.Encrypt("Gen token", ns.MaxRefreshTokenTTL, ns.MaxRefreshTokenTTL, ns.Name, userName, []byte(ns.RefreshKey))
