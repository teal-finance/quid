<template>
  <div class="flex flex-col space-y-5">
    <sw-input v-model:value="form.name.val" v-model:isvalid="form.name.isValid" :validator="form.name.validator"
      inline-label="Username" v-on:keyup.enter="search()" required autofocus></sw-input>
...
async function search() {
  ...
  try {
    const users = await AdminUser.searchNonAdmins(user.namespace.value.id, form.name.val)
    if (users.length == 0) {
