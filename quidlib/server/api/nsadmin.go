...
func AllAdministratorsInNamespace(w http.ResponseWriter, r *http.Request) {
	...
	if err := garcon.UnmarshalJSONRequest(w, r, &m); err != nil {
		logg.Warning("AllAdministratorsInNamespace:", err)
	...
	if err != nil {
		logg.QueryError("AllAdministratorsInNamespace: error selecting admin users:", err)
...
func SearchForNonAdminUsersInNamespace(w http.ResponseWriter, r *http.Request) {
	...
	if err := garcon.UnmarshalJSONRequest(w, r, &m); err != nil {
		logg.Warning("SearchForNonAdminUsersInNamespace:", err)
		w.WriteHeader(http.StatusBadRequest)
		return
	}
	...
	if p := garcon.Printable(username); p >= 0 {
		logg.Warning("SearchForNonAdminUsersInNamespace: JSON contains a forbidden character at p=", p)
		w.WriteHeader(http.StatusBadRequest)
		return
	}
	...
	if err != nil {
		logg.QueryError("SearchForNonAdminUsersInNamespace: error searching for non admin users:", err)
...
func CreateAdministrators(w http.ResponseWriter, r *http.Request) {
	...
	if err := garcon.UnmarshalJSONRequest(w, r, &m); err != nil {
		logg.Warning("CreateAdministrators:", err)
		w.WriteHeader(http.StatusBadRequest)
		return
	}
	...
	for _, uID := range uIDs {
		...
		if err != nil {
			logg.QueryError("CreateAdministrators: error checking admin user:", err)
			gw.WriteErr(w, r, http.StatusConflict, "error checking admin user")
			return
		}
		if exists {
			logg.QueryError("CreateAdministrators: admin user already exist:", err)
		...
		if _, err = db.CreateAdministrator(nsID, uID); err != nil {
			logg.QueryError("CreateAdministrators: error creating admin user:", err)
...
func DeleteAdministrator(w http.ResponseWriter, r *http.Request) {
	...
	if err := garcon.UnmarshalJSONRequest(w, r, &m); err != nil {
		logg.ParamError("DeleteAdministrator:", err)
		w.WriteHeader(http.StatusBadRequest)
		return
	}
	...
	if err != nil {
		logg.QueryError("DeleteAdministrator: error deleting admin users:", err)
