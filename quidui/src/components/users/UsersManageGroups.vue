  ...
  <div>
    <b-row class="mb-2">
      ...
      <b-col v-if="user.groups.size > 0">
        <b-badge variant="primary" class="mr-2" v-for="group in user.groups" :key="group.id">
          {{ group.name }}
          <b-icon icon="x-circle-fill" class="ml-1" @click="confirmRemoveUserFromGroup(group)"></b-icon>
        </b-badge>
      </b-col>
    ...
    <div v-else @click="addingToGroup=false">
      <users-add-to-group :user="user" @add-user-to-group="addUserToGroup($event)"></users-add-to-group>
    </div>
    <b-modal title="Remove user from group" ref="remove-from-group-modal">
      <span v-if="groupToRemove!==null">Remove user from group {{ groupToRemove.name }}?</span>
      <template v-slot:modal-footer="{ ok, cancel }">
        <b-button variant="danger" @click="removeUserFromGroup(groupToRemove)">Remove</b-button>
